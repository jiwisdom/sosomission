...
<script>
	$(function() {
		...
		$(document).on("click", ".page", function() {
			...
			let column = $("#column option:selected").val();
			let mean = $("#mean").val();
			showAllMember(page, column, mean);
		})

	})
	function showAllMember(page, column, mean) {
		$.ajax({
			url : "getMemberList",
			type : "get",
			data : {
			...
			success : function(data) {
				...
				$("#list").append("<tr id='tr'><th>id</th><th>pw</th><th>name</th><th>tel</th><th>email</th><th>reg_date</th></tr>");
				for (let i = 0; i < list.length; i++) {
					$("#list").append(
							"<tr><th>" + list[i].id + "</th><th width='10%'>"
									+ list[i].pw + "</th><th>"
									+ list[i].name + "</th><th>"
									+ list[i].tel + "</th><th>"
									+ list[i].email + "</th><th>"
									+ list[i].reg_date + "</th></tr>");
	...
	function getCount(count,page) {
		if(count>0){
			...
			$("#previousBlock").click(function(){
				if(page>5) {
					page_num-=5;
					showAllMember(page_num, column, mean);
				}
			})
			...
			if($("#column option:selected").val()=="all"){
				$("#under").append("<a> 가입된 회원 없음 </a>");
			}else if($("#column option:selected").val()=="id"){
				$("#under").append("<a> id가 "+ $("#mean").val()+"인 회원 없음</a>");
			}else if($("#column option:selected").val()=="name"){
				$("#under").append("<a> 이름이 "+ $("#mean").val()+"인 회원 없음</a>");
			}	
