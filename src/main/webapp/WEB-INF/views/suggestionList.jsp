...
<script>
	$(function() {
		getAllSugession("all", 1);

		$(document).on("click", ".page", function() {
			let page = $(this).attr("id");
			let searchVal = $("#search option:selected").val();
			getAllSugession(searchVal, page);
		})
	})

	function searchSuggestion() {
		let searchVal = $("#search option:selected").val();
		getAllSugession(searchVal, 1);
	}

	function getAllSugession(mean, page) {
		$("#listTbody").remove();
		$("#tbl").append("<tbody id='listTbody'></tbody>");
			$.ajax({
				...
				success : function(data) {
					...
					$("#listTbody")
							.append(
									"<tr id='tr'><th>번호</th><th>id</th><th>제목</th><th>상태</th><th>날짜</th></tr>");
		...
		<table id='tbl'>
			<tbody id='searchTbody'>
				<tr style='float: left;'>
					...
					<th><button onclick='searchSuggestion()'>검색</button></th>
				</tr>
			</tbody>
